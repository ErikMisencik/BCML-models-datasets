...
with open('One_does_not_simply_unclean.json', encoding='utf-8') as f:
    data = json.load(f)

    with open("One_does_not_simply_clean.json", "w", encoding='ascii', errors='ignore') as k:
        k.write('[\n')
    ...
    with open("One_does_not_simply_clean.json", "w", encoding='ascii', errors='ignore') as t:
        t.write('[\n')
        for i, item in enumerate(reformed_texts):
            if i == len(reformed_texts) - 1:
                t.write(f'["142434", "{item}|"]\n')
            else:
                t.write(f'["142434", "{item}|"],\n')
        t.write(']')
        print("done")